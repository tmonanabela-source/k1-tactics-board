...
(function (K1) {
  ...
  P.board = function (root) {
    ...
    s += section('Pitch', '<div class="choices">' + Object.values(K1.PITCHES).map(x => '<button class="choice' + (p.type === x.id ? ' on' : '') + '" data-pitch="' + x.id + '" title="' + esc(x.name) + '">' + esc(x.short) + '</button>').join('') + '</div>' +
      ...
      '<div class="row"><span class="row-label">Overlay</span><select class="select" data-pf="overlay">' + [['none', 'None'], ['thirds', 'Thirds'], ['lanes', '5 lanes (half-spaces)'], ['laneslab', '5 lanes, named'], ['zones15', 'Named lanes × thirds'], ['zones18', '18 zones'], ['zones20', '20 zones (lanes × 4)'], ['grid5', '5 m grid'], ['grid10', '10 m grid']].map(o => '<option value="' + o[0] + '"' + (p.overlay === o[0] ? ' selected' : '') + '>' + o[1] + '</option>').join('') + '</select></div>');
